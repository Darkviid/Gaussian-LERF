...
class SplatfactoModelConfig(ModelConfig):
    ...
    num_random: int = 5000 #Splatfacto: 50000
...
class SplatfactoModel(Model):
    ...
    def get_clip_relevancy(self, embeddings):
        ...
        positive_id = 0 # This value is the index of the positive class in the CLIP model
        clip_embeddings = self.clip_encoder.get_relevancy(embeddings, positive_id)
        return clip_embeddings

    def get_dino_relevancy(self, gaussian_outputs):
        dino_embeddings = self.dino_encoder(gaussian_outputs)
        return dino_embeddings

    def generate_heatmaps(self, clip_relevancy, dino_relevancy):
        # Genera los mapas de calor para CLIP y DINO
        clip_heatmap = torch.sigmoid(clip_relevancy).max(dim=-1)[0]
        dino_heatmap = torch.sigmoid(dino_relevancy).max(dim=-1)[0]

        return clip_heatmap, dino_heatmap
